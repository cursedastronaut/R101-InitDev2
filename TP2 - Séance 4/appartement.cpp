...
Appartement::Appartement ( ifstream &file ) {
	if (!file.is_open() && file.bad())
		throw string("File not opened!");
	file.clear();
	file.seekg(0);

	if (!file.eof()) {
		int nbPieces;
		getline(file, _adresse);
		file >> _zone;
		file >> nbPieces;
		for (int i = 0; i < nbPieces; ++i)
			_lesPieces.push_back(Piece(file, false));
	}
}

void Appartement::ecrireFichier(ofstream &file) {
	if (!file.is_open()) {
		throw string("Le fichier n'est pas ouvert.");
	}

	file << _adresse << endl << (int)_zone << endl << _lesPieces.size() << endl;
	for (size_t i = 0; i < _lesPieces.size(); ++i)
		_lesPieces[i].ecrireFichier(file);
}
